    ...
    def __init__(self, num_classes, trunk, use_aspp=False, num_filters=128):
        ...
        if self.use_aspp:
            ...
            self.aspp_conv2 = nn.Sequential(
                tltorch.FactorizedConv.from_conv(nn.Conv2d(high_level_ch, num_filters, 1, bias=False),rank=0.25, decompose_weights=True, factorization='tucker'),
                tltorch.FactorizedConv.from_conv(nn.Conv2d(num_filters, num_filters, 3, dilation=12, padding=12), rank=0.25, decompose_weights=True, factorization='tucker'),
                nn.BatchNorm2d(num_filters),
                nn.ReLU(inplace=True),
            )
            self.aspp_conv3 = nn.Sequential(
                tltorch.FactorizedConv.from_conv(nn.Conv2d(high_level_ch, num_filters, 1, bias=False), rank=0.25, decompose_weights=True, factorization='tucker'),
                tltorch.FactorizedConv.from_conv(nn.Conv2d(num_filters, num_filters, 3, dilation=36, padding=36), rank=0.25, decompose_weights=True, factorization='tucker'),
                nn.BatchNorm2d(num_filters),
                nn.ReLU(inplace=True),
            )
        ...
        self.conv_up1 = tltorch.FactorizedConv.from_conv(nn.Conv2d(aspp_out_ch, num_filters, kernel_size=1), rank=0.25, decompose_weights=True, factorization='tucker')
        self.conv_up2 = tltorch.FactorizedConv.from_conv(ConvBnRelu(num_filters + 64, num_filters, kernel_size=1).conv, rank=0.25, decompose_weights=True, factorization='tucker')
